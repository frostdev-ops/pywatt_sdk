...
impl EventInvalidationStrategy {
    ...
    pub async fn generate_key_async(
        ...
        if self.use_versioning {
            // Get the current version for this entity type
            let version_key = format!("{}:{}_version", self.prefix, entity_type);
            // Use 0 as delta to just read the current version without incrementing
...
impl InvalidationStrategy for EventInvalidationStrategy {
    fn generate_key(&self, entity_type: &str, entity_id: &str) -> String {
        if self.use_versioning {
            // Use a versioned prefix based on the entity type
            // Note: In actual usage, you'd get the version from versioned_key() method
            // but since this is a sync method, we'll use a consistent format
            format!("{}:{}:{}", self.prefix, entity_type, entity_id)
        } else {
            format!("{}:{}:{}", self.prefix, entity_type, entity_id)
        }
