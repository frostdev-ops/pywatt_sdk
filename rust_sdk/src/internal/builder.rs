...
impl<T: Send + Sync + Clone + 'static> ModuleBuilder<T> {
    ...
    /// Get a reference to the configured secret keys.
    pub fn get_secret_keys(&self) -> &[String] {
        &self.secret_keys
    }

    /// Get a reference to the configured endpoints.
    pub fn get_endpoints(&self) -> &[AnnouncedEndpoint] {
        &self.endpoints
    }

    /// Check if the state builder has been set.
    pub fn is_state_set(&self) -> bool {
        self.state_set
    }
...
mod module_builder_tests {
    ...
    use crate::communication::ipc_types::EndpointAnnounce;
    use crate::core::bootstrap::BootstrapError;
