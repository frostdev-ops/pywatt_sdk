...
impl InternalMessagingClient {
    /// Creates a new `InternalMessagingClient` specifically for IPC-only mode.
    /// This version doesn't attempt to establish a TCP connection.
    ///
    /// # Arguments
    ...
    pub fn new_for_ipc_only(
        ...
        // Log that we're operating in IPC-only mode
        ...
        // Create a dummy ConnectionConfig (it won't be used for connections)
        let config = crate::tcp_types::ConnectionConfig::new("127.0.0.1".to_string(), 0);
        let dummy_channel = TcpChannel::new(config);
        
        // Note: We could create a special NullChannel implementation here instead
        // but for simplicity we'll use TcpChannel with a known invalid address
        Self {
    ...
    pub fn new(
        ...
        // Create client with the provided channel or create a fallback channel
        let channel = orchestrator_channel.unwrap_or_else(|| {
            // Create a fallback channel for when no valid orchestrator_channel is provided
            // This allows the client to function in IPC-only mode or when TCP is not available
            tracing::warn!(
                module_id = %module_id, 
                "Creating InternalMessagingClient without a valid orchestrator_channel, creating fallback"
            );
            
            // Create a default connection config that won't actually be used for connections
            // but allows the structure to be created
            let fallback_config = crate::tcp_types::ConnectionConfig::new(
                "127.0.0.1".to_string(), 
                65535  // Use a high port number to indicate this is a fallback config
