...
impl IpcManager {
    ...
    /// Send a port negotiation request to the orchestrator using the new PortNegotiationManager
    pub async fn send_port_negotiation(&self, _request: IpcPortNegotiation) -> StdResult<(), String> {
        // We don't need this method anymore, as PortNegotiationManager handles sending requests internally
        // But we keep it for backward compatibility, delegating to the manager's internal methods
        debug!("Delegating port negotiation request to PortNegotiationManager");
        
        // The actual sending of the request is now handled by PortNegotiationManager
        // We just need to set up the request for the manager to use in request_port
        Ok(())
    }
    
    /// Wait for a port negotiation response from the orchestrator using PortNegotiationManager
    pub async fn wait_for_port_response(&self) -> StdResult<IpcPortNegotiationResponse, String> {
        // This is now handled by PortNegotiationManager internally
        // We implement it here for backward compatibility only
        debug!("Using PortNegotiationManager to wait for port response");
        
        // Check if we have an allocated port already
        ...
        // If we don't have a port, return an error
        Err("No port has been allocated by PortNegotiationManager".to_string())
    }
    
    /// Request a port from the orchestrator using PortNegotiationManager
    /// 
    /// # Arguments
    ...
    pub async fn request_port(&self, request: IpcPortNegotiation) -> StdResult<IpcPortNegotiationResponse, String> {
        // Create a PortNegotiationManager instance
        let port_manager = PortNegotiationManager::new();
        
        // Use the manager to request a port
        match port_manager.request_port(request.specific_port, None).await {
            Ok(port) => {
                // Port negotiation succeeded
                Ok(IpcPortNegotiationResponse {
            ...
            Err(e) => {
                ...
                error!("Port negotiation failed: {}", e);
                Ok(IpcPortNegotiationResponse {
                    request_id: request.request_id.clone(),
                    success: false,
                    port: 0, // We use 0 to indicate failure
                ...
                match serde_json::from_str::<OrchestratorToModule>(trimmed) {
                    Ok(msg) => {
                        ...
                        match msg {
                            ...
                            OrchestratorToModule::RegisterServiceProviderResponse(response) => {
                                safe_log!(
                                    info,
                                    "SDK IPC: Received RegisterServiceProviderResponse: success={}, provider_id={:?}",
                                    response.success,
                                    response.provider_id
                                );
                                // This would typically be handled by a pending request mechanism
                                // For now, we'll just log it
                            }
                            OrchestratorToModule::DiscoverServiceProvidersResponse(response) => {
                                safe_log!(
                                    info,
                                    "SDK IPC: Received DiscoverServiceProvidersResponse: success={}, provider_count={}",
                                    response.success,
                            ...
                            OrchestratorToModule::PortResponse(resp) => {
                                ...
                                // Use the PortNegotiationManager to process the port response
                                // This handles storing the port, notifying waiting tasks, and error handling
                                if let Err(e) = PortNegotiationManager::process_port_response(resp.clone()) {
                                    safe_log!(
                                        error,
                                        "SDK IPC: Error processing port response via PortNegotiationManager: {}",
                                        e
                                    );
                                } else {
                                    safe_log!(
                                        info,
                                        "SDK IPC: Successfully processed port response via PortNegotiationManager"
                                    );
                                }
                            }
